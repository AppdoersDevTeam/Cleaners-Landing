import { Link } from 'react-router-dom';
import { Phone, Mail, MapPin, Clock, ChevronDown } from 'lucide-react';
import { useState } from 'react';

const Footer = () => {
  const [openSection, setOpenSection] = useState<string | null>(null);

  const toggleSection = (section: string) => {
    setOpenSection(openSection === section ? null : section);
  };

  return (
    <footer className="bg-gray-200 text-gray-600">
      <div className="container-custom py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {/* Company info */}
          <div>
            <h2 className="text-xl font-bold mb-4 text-gray-800">Appdoers Cleaners</h2>
            <p className="mb-4 text-gray-600">Professional cleaning services for homes and businesses in New Zealand and surrounding areas.</p>
            <div className="flex space-x-4">
              <a href="https://facebook.com" target="_blank" rel="noreferrer" className="text-gray-600 hover:text-clean-blue transition-colors">
                <span className="sr-only">Facebook</span>
                <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path fillRule="evenodd" d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" clipRule="evenodd" />
                </svg>
              </a>
              <a href="https://instagram.com" target="_blank" rel="noreferrer" className="text-gray-600 hover:text-clean-blue transition-colors">
                <span className="sr-only">Instagram</span>
                <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path fillRule="evenodd" d="M12.315 2c2.43 0 2.784.013 3.808.06 1.064.049 1.791.218 2.427.465a4.902 4.902 0 011.772 1.153 4.902 4.902 0 011.153 1.772c.247.636.416 1.363.465 2.427.048 1.067.06 1.407.06 4.123v.08c0 2.643-.012 2.987-.06 4.043-.049 1.064-.218 1.791-.465 2.427a4.902 4.902 0 01-1.153 1.772 4.902 4.902 0 01-1.772 1.153c-.636.247-1.363.416-2.427.465-1.067.048-1.407.06-4.123.06h-.08c-2.643 0-2.987-.012-4.043-.06-1.064-.049-1.791-.218-2.427-.465a4.902 4.902 0 01-1.772-1.153 4.902 4.902 0 01-1.153-1.772c-.247-.636-.416-1.363-.465-2.427-.047-1.024-.06-1.379-.06-3.808v-.63c0-2.43.013-2.784.06-3.808.049-1.064.218-1.791.465-2.427a4.902 4.902 0 011.153-1.772A4.902 4.902 0 015.45 2.525c.636-.247 1.363-.416 2.427-.465C8.901 2.013 9.256 2 11.685 2h.63zm-.081 1.802h-.468c-2.456 0-2.784.011-3.807.058-.975.045-1.504.207-1.857.344-.467.182-.8.398-1.15.748-.35.35-.566.683-.748 1.15-.137.353-.3.882-.344 1.857-.047 1.023-.058 1.351-.058 3.807v.468c0 2.456.011 2.784.058 3.807.045.975.207 1.504.344 1.857.182.466.399.8.748 1.15.35.35.683.566 1.15.748.353.137.882.3 1.857.344 1.054.048 1.37.058 4.041.058h.08c2.597 0 2.917-.01 3.96-.058.976-.045 1.505-.207 1.858-.344.466-.182.8-.398 1.15-.748.35-.35.566-.683.748-1.15.137-.353.3-.882.344-1.857.048-1.055.058-1.37.058-4.041v-.08c0-2.597-.01-2.917-.058-3.96-.045-.976-.207-1.505-.344-1.858a3.097 3.097 0 00-.748-1.15 3.098 3.098 0 00-1.15-.748c-.353-.137-.882-.3-1.857-.344-1.023-.047-1.351-.058-3.807-.058zM12 6.865a5.135 5.135 0 110 10.27 5.135 5.135 0 010-10.27zm0 1.802a3.333 3.333 0 100 6.666 3.333 3.333 0 000-6.666zm5.338-3.205a1.2 1.2 0 110 2.4 1.2 1.2 0 010-2.4z" clipRule="evenodd" />
                </svg>
              </a>
            </div>
          </div>

          {/* Quick links */}
          <div>
            <div className="md:hidden">
              <button 
                onClick={() => toggleSection('quickLinks')}
                className="flex items-center justify-between w-full text-lg font-semibold mb-4 text-gray-800"
              >
                Quick Links
                <ChevronDown className={`h-5 w-5 transition-transform ${openSection === 'quickLinks' ? 'rotate-180' : ''}`} />
              </button>
            </div>
            <h3 className="hidden md:block text-lg font-semibold mb-4 text-gray-800">Quick Links</h3>
            <ul className={`space-y-2 ${openSection === 'quickLinks' ? 'block' : 'hidden'} md:block`}>
              <li><Link to="/" className="text-gray-600 hover:text-clean-blue transition-colors">Home</Link></li>
              <li><Link to="/services" className="text-gray-600 hover:text-clean-blue transition-colors">Services</Link></li>
              <li><Link to="/about" className="text-gray-600 hover:text-clean-blue transition-colors">About</Link></li>
              <li><Link to="/contact" className="text-gray-600 hover:text-clean-blue transition-colors">Contact</Link></li>
              <li><Link to="/booking" className="text-gray-600 hover:text-clean-blue transition-colors">Book Now</Link></li>
            </ul>
          </div>

          {/* Services */}
          <div>
            <div className="md:hidden">
              <button 
                onClick={() => toggleSection('services')}
                className="flex items-center justify-between w-full text-lg font-semibold mb-4 text-gray-800"
              >
                Services
                <ChevronDown className={`h-5 w-5 transition-transform ${openSection === 'services' ? 'rotate-180' : ''}`} />
              </button>
            </div>
            <h3 className="hidden md:block text-lg font-semibold mb-4 text-gray-800">Services</h3>
            <ul className={`space-y-2 ${openSection === 'services' ? 'block' : 'hidden'} md:block`}>
              <li><Link to="/services" className="text-gray-600 hover:text-clean-blue transition-colors">House Cleaning</Link></li>
              <li><Link to="/services" className="text-gray-600 hover:text-clean-blue transition-colors">Office Cleaning</Link></li>
              <li><Link to="/services" className="text-gray-600 hover:text-clean-blue transition-colors">Deep Cleaning</Link></li>
              <li><Link to="/services" className="text-gray-600 hover:text-clean-blue transition-colors">End of Lease Cleaning</Link></li>
              <li><Link to="/services" className="text-gray-600 hover:text-clean-blue transition-colors">Commercial Cleaning</Link></li>
            </ul>
          </div>

          {/* Contact info */}
          <div>
            <div className="md:hidden">
              <button 
                onClick={() => toggleSection('contact')}
                className="flex items-center justify-between w-full text-lg font-semibold mb-4 text-gray-800"
              >
                Contact Us
                <ChevronDown className={`h-5 w-5 transition-transform ${openSection === 'contact' ? 'rotate-180' : ''}`} />
              </button>
            </div>
            <h3 className="hidden md:block text-lg font-semibold mb-4 text-gray-800">Contact Us</h3>
            <ul className={`space-y-3 ${openSection === 'contact' ? 'block' : 'hidden'} md:block`}>
              <li className="flex items-start">
                <Phone className="h-5 w-5 mr-2 text-clean-blue mt-0.5" />
                <span className="text-gray-600">(03) 123 4567</span>
              </li>
              <li className="flex items-start">
                <Mail className="h-5 w-5 mr-2 text-clean-blue mt-0.5" />
                <span className="text-gray-600">info@appdoerscleaners.com</span>
              </li>
              <li className="flex items-start">
                <MapPin className="h-5 w-5 mr-2 text-clean-blue mt-0.5" />
                <span className="text-gray-600">123 Main Street, New Zealand, VIC 3147</span>
              </li>
              <li className="flex items-start">
                <Clock className="h-5 w-5 mr-2 text-clean-blue mt-0.5" />
                <span className="text-gray-600">Mon-Fri: 8am - 6pm<br />Sat: 9am - 2pm</span>
              </li>
            </ul>
          </div>
        </div>

        <div className="border-t border-gray-300 mt-8 pt-6 text-center text-gray-600 text-sm">
          <div className="flex flex-col md:flex-row items-center justify-between space-y-4 md:space-y-0">
            <img src="/logo.png" alt="Appdoers Limited Logo" className="h-6 w-auto" />
            <p>&copy; {new Date().getFullYear()} Appdoers Limited. All rights reserved.</p>
            <div>
              Website developed by <a href="https://appdoers.co.nz" target="_blank" rel="noopener noreferrer" className="text-gray-600 hover:text-clean-blue transition-colors">appdoers.co.nz</a>
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
};

export default Footer;
